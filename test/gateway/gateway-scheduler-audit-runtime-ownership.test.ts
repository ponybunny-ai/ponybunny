...
describe('GatewayServer scheduler-audit/runtime ownership split', () => {
  test('GatewayServer delegates scheduler-event audit observation wiring to the runtime helper', () => {
    ...
    expect(source).toContain('this.schedulerEventAuditObserver.start()');
    expect(source).toContain('this.escalationKnowledgeObserver.start()');
    expect(source).toContain('this.schedulerEventAuditObserver.stop()');
    expect(source).toContain('this.escalationKnowledgeObserver.stop()');
