...
import { DaemonBridge as ExplicitDaemonBridge } from '../../../src/gateway/integration/daemon-compatibility.js';
...
describe('gateway integration surface split', () => {
  ...
  test('compatibility barrel keeps historical exports explicit', () => {
    expect(integrationCompatibility.DaemonBridge).toBe(ExplicitDaemonBridge);
  ...
  test('historical daemon-bridge path stays a thin shim over explicit daemon compatibility', () => {
    expect(DaemonBridge).toBe(ExplicitDaemonBridge);
  });
});
