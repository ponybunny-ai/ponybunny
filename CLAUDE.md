...
- [Flywheel closed] PostGoalEvaluator now calls GlobalKnowledgeService.extractFromContextPack() after goal evaluation, closing the failure learning flywheel: Goal completes → PostGoalEvaluator evaluates → knowledge extracted from ContextPack → global_knowledge table → next Goal's Elaboration injects pitfalls. When adding new entry points, wire GlobalKnowledgeService into PostGoalEvaluator (via knowledgeDb parameter in assemblePostGoalEvaluator)
- When adding a new value to a TypeScript union type used in `Record<T, V>` (e.g., GoalStatus), check ALL files with `Record<GoalStatus, ...>` — formatters, state machine transitions, and UI icon maps will fail compilation with missing property errors. Also check SQL CHECK constraints.
- When relocating a file from `src/X/` to `src/X/Y/`, update BOTH the importing files AND the internal imports of the moved file — the relative depth changes (e.g., `../infra/` becomes `../../infra/`). Also fix inline `import('...')` type references deeper in the file, not just top-level imports.
