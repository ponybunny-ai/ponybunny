...
- [ADR-001 complete] Single execution architecture: GoalHarness (elaborate→plan) → SchedulerCore (execute with all production infra). All entry points (main.ts via HarnessDaemon, scheduler-daemon, gateway) route through GoalHarness. AutonomyDaemon removed in Phase 4. When adding lifecycle services, verify they are wired into GoalHarnessDependencies
