...
| RF-055 | Runtime tooling context extraction | Introduce an explicit scheduler-owned runtime tooling context and stop using `globalToolProvider` as the runtime-core source of truth | planned | medium | RF-032 | 54 | Recommended by Session 54 as the single next coding session: keep current tool execution semantics and worker seams, but move runtime-core tool/prompt capability ownership to an explicit injected boundary. |
