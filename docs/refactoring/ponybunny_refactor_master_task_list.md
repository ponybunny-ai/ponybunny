...
| RF-032 | Boundary cleanup | Reduce global singleton dependence in runtime core | in_progress | medium | RF-009 | 56 | Session 54 finalized the design baseline in `docs/refactoring/session54-runtime-core-boundary-cleanup-design.md`, Session 55 introduced the first explicit `RuntimeToolingContext` boundary for execution/planning/conversation consumers, and Session 56 continued the narrow cleanup by threading that context explicitly through the scheduler-daemon bootstrap/session-intake composition path while leaving gateway, IPC, worker seams, and durable ownership unchanged. |
