...
| RF-025 | Tool result handoff | Make tool result path scheduler/execution-safe and idempotent | planned | medium | RF-045 | 36 | Session 36 documented the handoff design first: recommend a local in-process authoritative worker awaited synchronously by `ReActIntegration`, with exact `toolRequestId` correlation and no tool mode switch in the first cut; implementation and later hardening remain pending. |
