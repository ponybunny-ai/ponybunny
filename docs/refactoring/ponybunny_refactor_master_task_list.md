...
| RF-025 | Tool result handoff | Make tool result path scheduler/execution-safe and idempotent | in_progress | medium | RF-045 | 38 | Session 38 hardened the authoritative local seam with request-identity validation, broader result-correlation checks, invalid-result normalization, and narrow in-process inspection visibility, but durable scheduler-safe idempotency and any evented handoff are still pending. |
