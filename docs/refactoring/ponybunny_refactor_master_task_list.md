...
| RF-032 | Boundary cleanup | Reduce global singleton dependence in runtime core | in_progress | medium | RF-009 | 57 | Session 54 finalized the design baseline in `docs/refactoring/session54-runtime-core-boundary-cleanup-design.md`, Session 55 introduced the first explicit `RuntimeToolingContext` boundary for execution/planning/conversation consumers, Session 56 threaded that context through the scheduler-daemon bootstrap/session-intake composition path, and Session 57 completed the next narrow tightening step by making the migrated daemon/session-intake path require explicit `RuntimeToolingContext` rather than allowing silent fallback while leaving gateway, IPC, worker seams, and durable ownership unchanged. |
