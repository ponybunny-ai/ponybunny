...
| RF-053 | Conversation timeout normalization | Implement the first narrow local timeout / missing-result resolution path for registered local conversation requests | planned | medium | RF-052 | — | Add one worker-owned local timer after authoritative registration in `ConversationWorker`, settle the registry-owned promise via the existing failure path with `CONVERSATION_EXECUTION_TIMEOUT`, preserve exactly one terminal outcome per `conversationRequestId`, and treat late completions as diagnostic-only without changing gateway behavior, IPC, continuation ownership, or scheduler-authoritative task materialization. |
