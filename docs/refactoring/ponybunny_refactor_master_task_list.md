...
| RF-025 | Tool result handoff | Make tool result path scheduler/execution-safe and idempotent | in_progress | medium | RF-045 | 37 | Session 37 switched the authoritative local dispatch seam from direct adapter execution to awaited `LocalToolWorker.dispatch(...)`, preserved `ReActIntegration` as the synchronous continuation owner, and added exact `toolRequestId` mismatch rejection at the worker boundary; broader scheduler-safe durability/idempotency and any evented handoff remain pending. |
