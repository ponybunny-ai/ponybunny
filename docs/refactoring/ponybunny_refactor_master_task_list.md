...
| RF-025 | Tool result handoff | Make tool result path scheduler/execution-safe and idempotent | in_progress | medium | RF-045 | 40 | Session 38 hardened the authoritative local seam with request-identity validation, broader result-correlation checks, invalid-result normalization, and narrow in-process inspection visibility; Session 40 then documented the promise-bridged authoritative handoff model required for any future less-direct path while preserving one awaited `ToolResult`, `toolRequestId` correlation, and `ReActIntegration` continuation ownership. |
...
| RF-047 | Tool promise bridge prototype | Prototype a narrow local internal request registry for promise-bridged authoritative tool handoff | planned | medium | RF-025 | 40 | Added by Session 40 as the single recommended next step: register pending tool requests by `toolRequestId`, own one promise per request, resolve exactly one terminal normalized `ToolResult`, and suppress duplicates without changing gateway behavior, IPC, or direct-mode continuation ownership. |
