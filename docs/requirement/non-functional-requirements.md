...
**需求**：系统必须支持足够的并发量以满足企业级使用。

**具体指标**：
- **并发Agent执行**：单节点至少8个Main Lane任务并行
- **消息吞吐**：
  - 每秒处理50+条入站消息（跨所有Channel）
  - 每秒发送100+条出站消息（流式输出）
- **WebSocket连接**：单实例支持500+并发连接
- **数据库操作**：SQLite WAL模式下，1000+ QPS读操作

**扩展策略**：
- 水平扩展：支持多Gateway实例（通过共享SQLite或分布式存储）
- 垂直扩展：利用多核CPU（Lane并发机制）
...
**需求**：系统必须持续可用，最小化计划外停机时间。

**具体指标**：
- **目标可用性**：99.5%（月停机时间 < 3.6小时）
- **单点故障容忍**：
...
**需求**：当单节点资源不足时，能够通过增加节点提升容量。

**架构支持**：
- **无状态Gateway**：Session数据存储在外部（共享NFS或S3）
- **负载均衡**：多Gateway实例共享WebSocket负载（通过Nginx/HAProxy）
- **分布式锁**：使用Redis实现Session锁（可选）
...
**当前限制**：
- SQLite不支持多节点并发写（需迁移到PostgreSQL）
- mDNS发现仅限局域网（需引入服务注册中心）
...
- **用户同意**：首次使用时明确隐私政策
- **数据导出**：用户可随时导出全部数据
- **数据删除**：用户可删除所有Session历史

**数据处理原则**：
- 默认本地存储，不上传到开发者服务器
- LLM API调用的数据处理符合各Provider的ToS
- 审计日志支持脱敏（自动隐藏敏感字段）
