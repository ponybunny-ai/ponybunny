...
**需求描述**：系统必须支持个人用户的多个Session（不同Channel、不同对话）完全隔离。

**业务价值**：
- **隐私分离**：工作对话与个人对话分离
- **多平台管理**：WhatsApp、Telegram、Slack等不同Channel的Session互不影响
- **小团队共享**：2-5人共享一个实例时，各自的Session隔离

**技术实现**：
- Session Key格式：`agent:{agentId}:{chatId}`
- 按Channel + User隔离状态
- Lane级别并发限制（个人用户：Main Lane 1-2并发，足够日常使用）

**验收标准**：
- [ ] 个人用户的工作Session无法访问个人Session的历史
- [ ] 同时在WhatsApp和Telegram使用，两个Session互不干扰
- [ ] 小团队（2-5人）共享实例时，各自Session完全隔离
- [ ] 单机支持10+并发Session（适合个人多设备或小团队）
...
**需求描述**：系统必须支持对不同类型任务使用不同成本的模型（如核心任务用强模型，简单任务用弱模型）。

**业务价值**：
- **成本降低**：个人用户月API开支从$20+降至$5-10
- **性能平衡**：复杂任务不妥协质量，简单任务节省开销
- **可持续性**：长期使用成本可控

**技术实现**：
- `agents.defaults.subagent.model`配置
- 任务分类：复杂推理（强模型 如Claude Sonnet） vs 简单任务（弱模型 如GPT-3.5）

**验收标准**：
- [ ] Subagent的LLM调用使用配置的弱模型
- [ ] 相比全强模型方案，个人用户月总Token消耗减少 > 60%
- [ ] 简单任务（文件读取、日志解析）成功率 > 90%
- [ ] 失败时自动failover到强模型
