...
**ADR-002**: SchedulerCore receives `ILogger` via constructor injection. All internal logging uses structured logger instead of `console.*`.
...
**ADR-002**: All lifecycle services receive `ILogger` via constructor injection.
...
3. Match against LLMErrorCode (ADR-002) → use code-based strategy
4. Check error.suggestedAction → use as fallback
5. Default: retry with same model (for recoverable errors)
```

### Error Pattern Matching (ADR-002: LLMErrorCode-based)

| LLMErrorCode | Recoverable | Strategy |
|-------------|------------|----------|
| `rate_limited` | Yes | same_model (with retryAfterMs) |
| `server_error` | Yes | same_model |
| `timeout` | Yes | same_model |
| `network_error` | Yes | same_model |
| `context_exceeded` | Yes | switch_model |
| `model_unavailable` | Yes | switch_model |
| `auth_failed` | No | escalate |
| `content_policy` | No | escalate |
| `quota_exceeded` | No | escalate |
...
**Source**: `src/runtime/react/react-integration.ts`
...
- **Tool execution timeout**: Enforced per-call (ADR-002 E2)
...
4. If tool_task → enter loop:
   a. Call LLM with native tool calling (via UnifiedLLMService)
   b. Parse response:
      - Text/thinking → record as thought
      - Task completion marker → break
      - User input request → pause
   c. Execute each tool call (with timeout enforcement):
...
Interface between scheduler and execution engine. All components receive `ILogger` (ADR-002).
...
Pub/sub event bus for loose coupling. All adapters receive `ILogger` (ADR-002).
...
## 6.13 PostGoalEvaluator (ADR-001 Phase 5 + ADR-002)
...
- `ILogger` — Structured logging (default: NoopLogger)
- `IMetricsRecorder` — Counter increments for goal completion, work item completion, escalation creation (default: NoopMetricsRecorder)
- Database persistence of GoalEvaluationReport to `goal_evaluation_reports` table

### Evaluation Flow

```
goal_completed / goal_failed / goal_blocked event received
  ...
  → Store report (bounded, max 100 in-memory)
  → Persist report to goal_evaluation_reports table
  → Record metrics (IMetricsRecorder)
  → Extract knowledge via GlobalKnowledgeService
  → Log summary
```
...
- **Bounded storage**: Last 100 reports in-memory, persisted to SQLite
...
**ADR-002**: HarnessDaemon flushes persistent metrics every 60 seconds via `PersistentSchedulerMetrics`. Metrics are restored on daemon restart.

## 6.15 Circuit Breaker (ADR-002)

**Source**: `src/infra/llm/circuit-breaker.ts`

Per-endpoint circuit breaker protecting LLM provider calls:
...
CircuitBreakerConfig {
  ...
  cooldownMs: 60_000        // Wait time before half-open probe
  monitoredErrorCodes: LLMErrorCode[]  // Default: server_error, timeout, network_error, rate_limited
}
```

### Behavior

| State | isCallAllowed() | On Success | On Failure |
|-------|----------------|------------|------------|
| **closed** | true | reset failure count | increment; if ≥ threshold → open |
| **open** | false (until cooldown) | — | — |
| **half-open** | true (one probe) | increment success; if ≥ threshold → closed | → open |

Health status exposed via `getCircuitBreakerHealth()` through `UnifiedLLMService.getProviderHealth()`.
