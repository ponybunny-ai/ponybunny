...
PonyBunny uses **SQLite** via `better-sqlite3` as its embedded database. The database uses a migration system with three versions:

- **Migration v1** — Bootstraps from `schema.sql` (core work order system) and `schema-memory.sql` (session and memory system)
- **Migration v2** — Creates `metric_counters` and `metric_samples` tables for persistent metrics (ADR-002)
- **Migration v3** — Creates `goal_evaluation_reports` table for PostGoalEvaluator persistence (ADR-002)

Legacy: `schema-migration-v2.sql` adds `allowed_actions` column to goals table (predates the migration system).
...
| status | TEXT DEFAULT 'pending' | queued, plan_review, active, blocked, completed, cancelled |
| priority | INTEGER DEFAULT 50 | 0-100 |
| allowed_actions | TEXT | JSON array (added in migration v2-legacy) |
...
**ADR-002**: RuntimeEventTracer writes span data to this table. Spans include attributes, events, duration, and status in `payload_json`.
...
| id | TEXT PK | UUID v4 |
| created_at | INTEGER | |
| source_goal_id | TEXT | FK to goals |
| source_context_pack_id | TEXT | FK to context_packs |
| knowledge_type | TEXT NOT NULL | pitfall, pattern, approach, decision |
...
**Indexes**: knowledge_type, (knowledge_type, confidence DESC), source_goal_id

## 4.3 Metrics Tables (ADR-002 Migration v2)

### metric_counters

| Column | Type | Notes |
|--------|------|-------|
| name | TEXT NOT NULL | Composite PK with labels |
| labels | TEXT NOT NULL | JSON object (serialized label key-values) |
| value | REAL NOT NULL | Counter value (incremented via ON CONFLICT) |
| updated_at | INTEGER NOT NULL | |

**Primary Key**: (name, labels)

Used by `SQLiteMetricsRecorder.increment()` — upserts with ON CONFLICT to atomically add to value.
...
| value | REAL NOT NULL | Gauge or duration sample |
| recorded_at | INTEGER NOT NULL | |

**Indexes**: (name, recorded_at)

Used by `SQLiteMetricsRecorder.gauge()` and `recordDuration()`.

## 4.4 Goal Evaluation Reports Table (ADR-002 Migration v3)

### goal_evaluation_reports
...
| created_at | INTEGER NOT NULL | |
| goal_id | TEXT NOT NULL | FK to goals |
| trigger | TEXT NOT NULL | CHECK: 'goal_completed', 'goal_failed', 'goal_blocked' |
| work_item_results | TEXT | JSON array of WorkItemEvaluation |
| summary_total | INTEGER DEFAULT 0 | |
| summary_publish | INTEGER DEFAULT 0 | |
| summary_retry | INTEGER DEFAULT 0 | |
| summary_replan | INTEGER DEFAULT 0 | |
| summary_escalate | INTEGER DEFAULT 0 | |
| summary_skipped | INTEGER DEFAULT 0 | |
| unactionable_decisions | TEXT | JSON array of strings |

**Indexes**: goal_id, created_at DESC

Persisted by PostGoalEvaluator after each goal completion/failure/block event. Queryable via `evaluation.list` and `evaluation.get` RPC methods.

## 4.5 Authentication & Authorization Tables
...
| action | TEXT NOT NULL | Prefixed format: `{source}.{entity}.{verb}` (ADR-002) |
...
LRU cache for embedding vectors. ADR-002 adds in-memory `EmbeddingLruCache` layer (max 500 entries) in front of this table.
...
## 4.8 Repository Interface Summary
...
**Database Access**: `getDatabase()` — Exposes raw database for cross-cutting services (GlobalKnowledgeService, SQLiteMetricsRecorder, RuntimeEventTracer)
...
## 4.9 Migration System

**Source**: `src/infra/persistence/migrations/index.ts`
...
const MAIN_DB_MIGRATIONS: Migration[];    // v1: schema.sql, v2: metric tables, v3: evaluation reports
const MEMORY_DB_MIGRATIONS: Migration[];  // v1: schema-memory.sql
```

Migrations are additive (IF NOT EXISTS) for safe re-runs. Applied by `DatabaseMigrator` at startup in both main.ts and gateway bootstrap.

## 4.10 Error Signature Computation
