...
### Plan Review (read/write) — New

| Method | Permission | Params | Returns |
|--------|-----------|--------|---------|
| `plan.get` | read | `{ goalId }` | `{ workItems }` — planned work items for goal in `plan_review` status |
| `plan.approve` | write | `{ goalId }` | `{ success }` — transitions `plan_review` → `active` |
| `plan.reject` | write | `{ goalId, reason? }` | `{ success }` — transitions `plan_review` → `cancelled` |
...
### Evaluation Reports (read) — New

| Method | Permission | Params | Returns |
|--------|-----------|--------|---------|
| `evaluation.list` | read | `{ goalId?, limit? }` | `{ reports: GoalEvaluationReport[] }` |
| `evaluation.get` | read | `{ goalId }` | `{ report: GoalEvaluationReport }` — latest report for goal |

### Harness Metrics (read) — New

| Method | Permission | Params | Returns |
|--------|-----------|--------|---------|
| `harness.errorClusters` | read | `{ limit? }` | Error signature clusters from runs |
| `harness.failureRates` | read | `{}` | Failure rates by work-item type |
| `harness.timeline` | read | `{ days? }` | Daily goal success/failure timeline |
| `harness.evaluationDistribution` | read | `{}` | Aggregate evaluation decision counts |
| `harness.knowledgeStats` | read | `{}` | Global knowledge store effectiveness |
...
- **Reliability**: Auto-reconnect with exponential backoff (1s → 30s), BackpressureBuffer (max 1000, drop oldest) (ADR-002)
