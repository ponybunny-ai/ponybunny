...
├──────────────────────────────────────────────────────────────────┤
│                      Autonomy Layer                              │
│  src/autonomy/     ReAct loop, daemon mode                       │
├──────────────────────────────────────────────────────────────────┤
│                    Application Layer                              │
...
|-------|----------------|-------------------|
| **Domain** (`src/domain/`) | Nothing (pure) | app, infra, gateway, scheduler, runtime, cli |
| **App** (`src/app/`) | domain | infra, gateway, scheduler, runtime, cli |
| **Infra** (`src/infra/`) | domain, app | gateway, scheduler, cli |
| **Runtime** (`src/runtime/`) | domain, app, infra | gateway, cli |
| **Scheduler** (`src/scheduler/`) | domain, app, infra, runtime | gateway, cli |
| **Gateway** (`src/gateway/`) | domain, app, infra, scheduler, runtime, ipc | cli |
...
│   ├── daemon.ts              #   Legacy autonomy daemon
│   └── daemon-event-emitter.ts
│
├── scheduler-daemon/          # Daemon process bootstrap
│   ├── bootstrap/             #   Default daemon runtime factory
...
## 2.4 Data Flow: Goal Submission to Completion
...
└────────────────────────────┬──────────────────────────┘
                             │ IPC
┌─ Scheduler ────────────────▼──────────────────────────┐
│  6. submitGoal() → creates GoalExecutionState         │
...
└────────────────────────────┬──────────────────────────┘
                             │ IPC events
┌─ Gateway ──────────────────▼──────────────────────────┐
│  11. Broadcasts events to subscribed clients:         │
