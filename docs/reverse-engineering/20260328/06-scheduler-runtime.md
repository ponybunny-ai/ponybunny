...
The `SchedulerCore` is the central orchestration engine. It processes goals through a tick-based loop, managing work item execution, budget enforcement, and error recovery.
...
|-------|---------|---------------|
| **Intake** | IntakeService | Validate goal structure, create Goal entity, check invariants |
| **Elaboration** | ElaborationService | Expand goal requirements, identify ambiguities |
| **Clarification** | ClarifyService | Generate clarification questions, process user responses |
| **Planning** | PlanningService | Decompose goal into work items (DAG), set dependencies |
| **Execution** | ExecutionService | Run ReAct loop with LLM + tools |
| **Verification** | VerificationService | Run quality gates (deterministic + LLM review) |
| **Evaluation** | EvaluationService | Decide: publish, retry, replan, or escalate |
| **Publish** | PublishService | Package artifacts, generate summaries |
| **Monitor** | MonitorService | Track metrics, detect stuck states, budget alerts |
