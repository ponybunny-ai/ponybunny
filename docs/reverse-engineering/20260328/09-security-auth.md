...
Credentials stored in `~/.config/ponybunny/credentials.json`:
...
Every state mutation is recorded in `audit_logs`:
...
AuditLog {
  ...
  action: string          // goal.create, workitem.update, permission.grant, etc.
...
Operational errors (`console.error`) are never gated.
